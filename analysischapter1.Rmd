...
```{r}
perm.p <- adonis2(dannevig.bc.data~temp*sal*par*fluo*chla,
                  data=env,permutations=999)
...
```{r}
mod1 <- dbrda(otu ~  temp+sal + par + fluo + dens + chla, 
             data = env, 
             distance = "bray",
             na.action=na.omit)
vif.cca(mod)
...
mod2 <- dbrda(otu ~  temp + sal + par + fluo ,
             data = env, 
             distance = "bray",
             na.action=na.omit)
vif.cca(mod2)

mod <- dbrda(otu ~  temp + sal + chla + fluo ,
             data = env, 
             distance = "bray",
             na.action=na.omit)
vif.cca(mod)
...
anova(mod2)   
# test each environmental factor
anova(mod2, by="term")
# summary of dbRDA model to extract total variance constrained and axis scores
summary(mod1)
RsquareAdj(mod2)
scores(mod2, display = "bp")


env.2 <- subset(env,sample!="O1-0m-21")
env.2 <- subset(env.2,sample!="O1-Deep-21")
env.2 <- subset(env.2,sample!="O1-5m-21")
env.2 <- subset(env.2,sample!="O1-15m-21")
env.2 <- subset(env.2,sample!="K1-0m-21")
env.2 <- subset(env.2,sample!="K1-5m-21")
env.2 <- subset(env.2,sample!="K1-Deep-21")
env.2 <- subset(env.2,sample!="K1-15m-21")
env.2 <- subset(env.2,sample!="K1-Deep-22")

# Extract site scores (sample positions)
sites <- scores(mod, display = "sites") %>% as.data.frame()
sites$depth <- as.factor(env.2$depth)   # categorical depth variable in your env data
sites$year  <- as.factor(env.2$year)
sites$station <- as.factor(env.2$station)
...
palette.rda <- c("#ff8811","#a1c181","#CD1C1C","#46237a")
  
gg <- ggplot() +
  geom_point(data = sites, aes(x = dbRDA1, 
                               y = dbRDA2, 
                               color = fct_reorder(depth,env.2$drange), 
                               shape= year,
                               label=station), size = 3) +
  scale_color_manual(values = palette.rda) +
  scale_shape_manual(values = c(16, 21)) +
#  stat_ellipse(data = sites, aes(x = dbRDA1, y = dbRDA2), 
#               geom="polygon", alpha=0.3,level = 0.95) +
  geom_segment(data = species,
               aes(x = 0, y = 0, xend = dbRDA1, yend = dbRDA2),
               arrow = arrow(length = unit(0.3,"cm"),type="open"), color = "blue") +
  geom_text(data = species,
            aes(x = dbRDA1, y = dbRDA2, label = rownames(species)),
            vjust = -0.8, size = 4,color = "blue" ) +
...
## Save plot
```{r}
jpeg("D:/plotspaper/dbrda6.jpeg", # CHANGE directory
     width = 2400,
     height = 1600,
     res = 400)
gg
