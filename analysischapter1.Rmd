...
```{r}
env<-alpha_diversity[11:17]
envsz<-as.data.frame(scale(env))

```

```{r}
otun15<-as.data.frame(otu_table(protists_n15))
...
```{r}
palette1=c("#6DA34D","#12355B","#8DA7BE","#EB5160","#DE9E36")


paletteall=c("#332288","#6699CC","#88CCEE","#44AA99","#117733","#999933","#DDCC77",
 "#C18748","#AF4E24","#661100","#B40F20","#AA4499")
...
## Estimate richness of the dataset

```{r}

alpha <- estimate_richness(rare) #Calculates all the indexes for alpha diversity 
...
alpha <- write.xlsx(alpha,"D:/dannevig/datafiles/alphadiversity.xlsx" ) #Saving the alpha diversity calculations as an excel file
...
alpha$year[alpha$year=="1"]<-"2021"
alpha$year[alpha$year=="2"]<-"2022"
...
```{r}
kruskal.test(Shannon~chla,data = alpha_diversity)

kruskal.test(Shannon~fluo,data = alpha_diversity)

```

```{r}
model<-lme4::lmer(Shannon~temp+sal+fluo,data=alpha_diversity)

model_anova<-aov(model)
summary(model_anova)

tukey.test<-TukeyHSD(model_anova)
```
...
lm<-lme4::lme
r(Shannon~depth+station+(1|year),data=alpha)
plot(lm)
summary(lm)
model_anova2<-aov(lm)
...
p1 = plot_ordination(rare.nmds, GP.ord, type="taxa", color="station",shape="depht")
print(p1)

dannevig.bc.15 <- vegdist(otu.rare,'bray')    #Create Bray-Curtis matrix
dannevig.mds <- metaMDS(dannevig.bc.15,k=2,autotransform=FALSE) 
stressplot(dannevig.mds, main="Shepard plot")
dannevig.mds

envn15<-read.xlsx("D:/dannevig/datafiles/env_n15.xlsx")
envn15.b<-envn15[8:14]
envn15.b[is.na(envn15.b)]<-0
envn15.s<-as.data.frame(scale(envn15.b)) #scale the parameters
fit2 <- envfit(dannevig.mds, env15.s)

env[is.na(env)]<-0
env.s<-as.data.frame(scale(env)) #scale the parameters
fit <- envfit(dannevig.mds, env.s)
#run the following three lines together
ordiplot(dannevig.mds$points,display="sites",type="n")
points(dannevig.mds$points,pch=as.character(dannevig$depth))#plotting sites in function of depth
plot(fit1)
...
palette_stations_fill <- c("#332288", "#6699CC", "#88CCEE", "#44AA99", "#117733", "#999933", 
                           "#F47B87","#E21227","#830B17","#DDCC77", "#C18748", "#AF4E24")#goes in alphabetical order to assign the colors

# Add a new column for conditional fill
data.scores$fill_color <- ifelse(data.scores$year == "2021",
                                 palette_stations_fill[data.scores$station],  # Use station colors for 2021
                                 NA)  # No fill for 2022


# Define shape mappings for 3 depths
shapes_depth <- c(21, 22, 23)  # Choose shapes that support fill
shapes_depth <- c(19,16,17)  # Choose shapes that support fill

# Plot with conditional fill and other aesthetics

data.scores.n15<-subset(data.scores,depth!="15m")

gg <- ggplot(data = data.scores.n15, aes(x = NMDS1, y = NMDS2,
                                     shape=year,
                                     color=depth)
                                    ) +
   #geom_jitter(
   # aes(
   #   color =fct_reorder(station,srange), # Color by station (12 unique values)
    #  shape = depth,           # Shape by depth (3 unique values)
   #   fill = fill_color# Conditional fill color based on year
   # ),
  #  size = 2, alpha = 0.7
  #) +
  geom_point(size=1)+
  #stat_ellipse(aes(group = year, color=year), type = "t", linetype = "dashed", size = 0.5) +
  geom_text(data=data.scores.n15,aes(x=NMDS1,y=NMDS2,label=station),
            alpha=0.5,color="black",size=2,vjust=0,hjust=0)+
  #scale_colour_manual(values =palette_stations) +  # Set custom colors for stations
 # scale_shape_manual(values = shapes_depth) +       # Set shapes for depths
  #scale_fill_identity(guide = "none") +             # Directly use fill_color values, no separate legend
  guides(
    color = guide_legend(title = "Depth",override.aes = list(size = 1)),
    shape = guide_legend(title = "Year",override.aes = list(size = 1))
      ) +
  theme(
...
plot(dispersion, hull = FALSE, ellipse=TRUE,label = T,
     col = c("darkorchid1",  "brown1", "orange","green"))
        #col=c("#332288","#6699CC","#88CCEE","#44AA99","#117733","#999933","#DDCC77","#C18748","#AF4E24","#661100","#B40F20","#AA4499"))
legend("right",title="Depth",
       c("0m","15m","90m"),
       #c("Jan","Feb","March","Apr","May","June","July","August","Sept","Oct","Nov","Dec"),
       #fill =c("#332288","#6699CC","#88CCEE","#44AA99","#117733","#999933","#DDCC77","#C18748","#AF4E24","#661100","#B40F20","#AA4499"),
       fill=c("darkorchid1",  "brown1", "orange","green"),
       horiz=FALSE, cex=.9)

jpeg("D:/plotspaper/nmds10.jpeg",
     width = 2400,
     height = 1400,
     res = 500)
gg
...
env<-read.xlsx("D:/dannevig/datafiles/env.xlsx")
...
protists.permanova <- adonis2(dannevig.bc~transect*station*depth*year,data=env,permutations=10000)

print(protists.permanova)   
...
protists21.permanova <- adonis2(dannevig.bc.21~.,data=env21.s,permutations=999)
print(protists21.permanova) 
